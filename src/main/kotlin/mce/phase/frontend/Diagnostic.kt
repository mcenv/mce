...
sealed class Diagnostic {
    ...
    companion object {
        fun serializeTerm(term: C.Term): S.Term = when (term) {
            ...
            is C.Term.Or -> S.Term.Or(term.variants.map { serializeTerm(it) }, term.id ?: freshId())
            is C.Term.And -> S.Term.And(term.variants.map { serializeTerm(it) }, term.id ?: freshId())
            ...
            is C.Term.Compound -> S.Term.Compound(term.elements.map { (name, element) -> S.Entry(element.relevant, name, serializeTerm(element.type), element.id ?: freshId()) }, term.id ?: freshId())
            ...
            is C.Term.Fun -> S.Term.Fun(
                term.parameters.map { S.Parameter(it.termRelevant, it.name, it.lower?.let(Companion::serializeTerm), it.upper?.let(Companion::serializeTerm), it.typeRelevant, serializeTerm(it.type), it.id) },
