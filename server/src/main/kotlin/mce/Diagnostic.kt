...
sealed class Diagnostic {
    ...
    companion object {
        fun serializeTerm(term: C.Term): S.Term = when (term) {
            ...
            is C.Term.CompoundOf -> S.Term.CompoundOf(term.elements.map { serializeTerm(it) }, term.id ?: freshId())
            ...
            is C.Term.List -> S.Term.List(serializeTerm(term.element), term.id ?: freshId())
            is C.Term.Compound -> S.Term.Compound(term.elements.map { it.first to serializeTerm(it.second) }, term.id ?: freshId())
        ...
        fun serializePattern(pattern: C.Pattern): S.Pattern = when (pattern) {
            ...
            is C.Pattern.CompoundOf -> S.Pattern.CompoundOf(pattern.elements.map { serializePattern(it) }, pattern.id)
