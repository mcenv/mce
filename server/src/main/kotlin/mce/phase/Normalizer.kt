...
class Normalizer(
    ...
    fun eval(term: C.Term): C.Value = when (term) {
        ...
        is C.Term.List -> C.Value.List(lazy { eval(term.element) }, lazy { eval(term.size) }, term.id)
    ...
    fun quote(value: C.Value): C.Term = when (val value = force(value)) {
        ...
        is C.Value.List -> C.Term.List(quote(value.element.value), quote(value.size.value), value.id)
