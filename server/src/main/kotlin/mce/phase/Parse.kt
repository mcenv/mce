...
class Parse private constructor(source: String) {
    ...
    private fun Node.parseTerm(): Term = when (type) {
        ...
        "term:boolean_of" -> Term.BooleanOf(parseBoolean(), id)
        "term:byte_of" -> Term.ByteOf(parseByte(), id)
        "term:short_of" -> Term.ShortOf(parseShort(), id)
        "term:int_of" -> Term.IntOf(parseInt(), id)
        "term:long_of" -> Term.LongOf(parseLong(), id)
        "term:float_of" -> Term.FloatOf(parseFloat(), id)
        "term:double_of" -> Term.DoubleOf(parseDouble(), id)
        "term:string_of" -> Term.StringOf(parseString(), id)
